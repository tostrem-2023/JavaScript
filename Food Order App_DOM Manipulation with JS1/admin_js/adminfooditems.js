...
var foodCollection = JSON.parse(window.localStorage.getItem("Food Item")); 
var foodId = 0;
...
function saveFoodItem(foodId, fname, desc, inStock, isVeg, category, cuisine, img){
    ...
    var fooditemData = JSON.stringify(fooditem);
    window.localStorage.setItem("Food Item", fooditemData);
};

//To load all available categories in Add Food Page
//View All Food Items Info
function listFoodItems(){
    let fooditemTable = document.getElementById("fooditemstable");
    if(foodCollection !=null){
